https://getlinkfshare.com/
https://vnlinks.net/
http://www.linkvip.info/
http://ilikesub.com/get-link-chia-se-nhac

http://txt.do/53bv4

http://www.ebook777.com/
http://mirror.ufs.ac.za/
https://sourceforge.net/projects/thumbapps/files/
http://thuvienphapluat.vn/
http://www.giaiphapexcel.com/forum/forum.php
http://www.oxfordlearnersdictionaries.com
http://macgund.blogspot.com/
http://i.youku.com/i/UNTE0NTY0MjA=/videos?spm=a2hzp.8244740.0.0
http://i.youku.com/i/UMTE2MzM0MDg=/videos?spm=a2hzp.8244740.0.0
https://traibap.wordpress.com/2011/06/08/76/
https://www.fshare.vn/folder/RNN23YCG3Q8L
http://tve-4u.org/threads/bo-gia-mario-puzo-doan-tu-huyen-trinh-huy-ninh.27205/
http://tieba.baidu.com/p/3840006456
http://tieba.baidu.com/p/3263526797
http://tieba.baidu.com/p/2741106090
http://tieba.baidu.com/p/4853103946
https://www.fshare.vn/file/T60H1HW3BT
https://www.tenlua.vn/download/1737e42ee60a6503/celtic-woman-emerald-musical-gems-2013-blu-ray-1080i-avc-dts-hd-5-1-iso
http://pan.baidu.com/share/link?shareid=447914139&uk=773140592&adapt=pc&fr=ftw#path=%252F
http://www.lightnovel.cn/thread-835719-1-1.html

http://cmder.net/
http://www.ltr-data.se/opencode.html/#ImDisk
http://www.mitec.cz/tmx.html
http://www.hexagora.com/en_dw_davperf.asp
http://www.bandisoft.com/honeyview/
http://www.revouninstaller.com/revo_uninstaller_free_download.html
http://unikey.org/bdownload.php
https://notepad-plus-plus.org/
https://sourceforge.net/projects/npp-customize/
http://free.kaspersky.com/
http://z-o-o-m.eu/
https://otter-browser.org/
https://ftp.mozilla.org/pub/firefox/
https://www.mozilla.org/en-US/firefox/organizations/all/
about:config > browser.tabs.closeWindowWithLastTab boolean false
               extensions.firebug.hideDefaultInspector boolean true
			   devtools.inspector.enabled boolean false
			   browser.tabs.tabClipWidth integer 1
https://addons.mozilla.org/en-US/firefox/addon/adblock-plus/?src=search
https://addons.mozilla.org/en-US/firefox/addon/flashgot/?src=search
https://addons.mozilla.org/en-US/firefox/addon/downthemall/?src=search
https://addons.mozilla.org/en-US/firefox/addon/righttoclick/
https://addons.mozilla.org/vi/firefox/addon/clippings/
https://addons.mozilla.org/en-US/firefox/addon/greasemonkey/
https://greasyfork.org/en/scripts/23635-%E7%99%BE%E5%BA%A6%E7%BD%91%E7%9B%98%E7%9B%B4%E6%8E%A5%E4%B8%8B%E8%BD%BD%E5%8A%A9%E6%89%8B
https://greasyfork.org/scripts/7382-baidupandownloadhelper-modify-by-ted423/code/BaiduPanDownloadHelper%20modify%20by%20ted423.user.js
https://vozforums.com/showpost.php?p=60840624&postcount=104961
https://addons.mozilla.org/en-US/firefox/addon/stylish/?src=external-userstyleshome
https://addons.mozilla.org/en-US/firefox/addon/context-search/?src=search
https://addons.mozilla.org/en-US/firefox/addon/add-to-search-bar/?src=search
https://addons.mozilla.org/en-US/firefox/addon/cookie-wizard-with-whitelist/?src=userprofile
http://www.xuldev.org/firegestures/
https://chromium.woolyss.com/
https://www.google.com/intl/en/chrome/browser/desktop/index.html?standalone=1&platform=win64
http://www.slimjet.com/
start UCbrowser.exe --user-data-dir="Portable"
--user-data-dir= --ppapi-flash-path= --allow-outdated-plugins
http://chrome-extension-downloader.com
https://chrome.google.com/webstore/detail/newtab-clear/dgjhckfnkeboglnhifjdbpakiocmophg?hl=en
https://chrome.google.com/webstore/detail/auto-history-wipe/hdgnienkeomlaeeojaibeicglpoaadnj
https://chrome.google.com/webstore/detail/last-tab/nggjcpipkefkgldicofgmealjndjnhba?hl=en-US
https://chrome.google.com/webstore/detail/closed-tabs/eonffnnfmbfnmjpaiigdclmfelolemah
https://chrome.google.com/webstore/detail/bulk-media-downloader/ehfdcgbfcboceiclmjaofdannmjdeaoi?hl=en
https://chrome.google.com/webstore/detail/chrono-download-manager/mciiogijehkdemklbdcbfkefimifhecn?hl=en
https://chrome.google.com/webstore/detail/permanent-clipboard/hilkjcfodmbdgpadbpehimibheopoccb?hl=en
https://chrome.google.com/webstore/detail/violent-monkey/jinjaccalgkegednnccohejagnlnfdag
https://chrome.google.com/webstore/detail/stylish/fjnbnpbmkenffdnngjfgmeleoegfcffe
https://chrome.google.com/webstore/detail/context-menu-search/ocpcmghnefmdhljkoiapafejjohldoga?hl=en-US
http://www.xuldev.org/firegestures/
"C:\Program Files\Internet Explorer\iexplore.exe" -private
https://github.com/gorhill/uBlock#installation
https://raw.githubusercontent.com/nmtrung/FMSF-2.0/master/fmsf_2.0.txt
https://raw.githubusercontent.com/nmtrung/FMSF-3x/master/fmsf-3x-filters.txt
https://easylist-downloads.adblockplus.org/easylist.txt
http://abpvn.com/
https://raw.githubusercontent.com/Yhonay/antipopads/master/popads.txt
https://helpx.adobe.com/flash-player/kb/archived-flash-player-versions.html
http://kmeleonbrowser.org/forum/read.php?1,114856,page=1
http://www.qbittorrent.org/download.php
http://www.rarlab.com/download.htm
http://www.7-zip.org/
https://www.tracker-software.com/product/pdf-xchange-viewer
http://www.sumatrapdfreader.org/download-free-pdf-viewer.html
http://www.adobe.com/support/downloads/product.jsp?product=10&platform=Windows
http://www.cutepdf.com/products/cutepdf/writer.asp
http://www.ghostscript.com/download/
http://www.lingoes.net/en/translator/download.htm
http://www.jaist.ac.jp/~hoangle/filesj/
http://ejie.me/
http://www.extendoffice.com/download/office-tab-free-edition.html
http://www.java.com/en/download/manual.jsp
https://mpv.srsfckn.biz/
C:\Users\aa\Documents\WW1\mpv-i686-20160411\mpv.com
[URL] --user-agent=[UA] --cookies-file=[CFILE] --referrer=[REFERER]
https://potplayer.daum.net/
C:\Windows\System32\cmd.exe
/C <NUL set /p=[URL]| clip
C:\Windows\System32\cmd.exe
/C echo [URL] >> C:\Users\aa\Desktop\1.txt
C:\Windows\System32\cmd.exe
/C
(echo url=[URL]
echo referer=[REFERER]
echo cookie=[COOKIE]
) >> C:\Users\aa\Desktop\1.txt
https://st0rage.org/~n2j3/?page_id=225717166
C:\Users\aa\Desktop\axel.exe
-n 10 [-U UA] [-o C:\Users\aa\Desktop\FNAME] [-H Cookie:COOKIE] [-H Referer:REFERER] [URL]
http://westbyte.com/ida/
C:\Users\aa\Documents\ \WW3\IDA Portable\ida.exe
[URL] [referer=REFERER] [description=COMMENT] [savepath=FOLDER] [filename=FNAME] [cookies=COOKIE] [post=POST]
https://www.gnu.org/software/wget/
C:\Users\aa\Desktop\wget.exe
-c [--referer=REFERER] [--post-data=POST] [--load-cookies=CFILE] [--header=Cookie:COOKIE] [--output-document=C:\Users\aa\Desktop\FNAME] [--input-file=UFILE] [URL]
http://forum.eagleget.com/
@Echo off
SETLOCAL ENABLEDELAYEDEXPANSION
Set A=
Set B=
Set C=
Set D=
Set E=
Set F=
"C:\Users\aa\Desktop\EagleGetProtable_2.0.4.20\EagleGet.exe" \S \U!A:~1,-1!U\ \R!B:~1,-1!R\ \C!C:~1,-1!C\ \H!D:~1,-1!H\ \G!E:~1,-1!G\ \P!F:~1,-1!P\
http://forum.eagleget.com/viewtopic.php?id=643
C:\Windows\System32\cmd.exe
/C Start C:\Users\FG.cmd [URL]
@Echo off
SETLOCAL ENABLEDELAYEDEXPANSION
Set Parm="%~1"
Set CleanURL=%Parm: =%
Echo !CleanURL:~1,-1!  >> "%ProgramFiles%\EagleGetProtable_2.0.4.11\FGLog.txt"
"%ProgramFiles%\EagleGetProtable_2.0.4.11\EagleGet.exe" \S \U!CleanURL:~1,-1!U\
Exit
http://www.internetdownloadmanager.com/download.html
http://www.internetdownloadmanager.com/idmmzcc/idmmzcc.xpi
https://github.com/rg3/youtube-dl
https://github.com/soimort/you-get
http://txpress.blogspot.com/2007/01/join-multiple-video-mpg-files-in.html
http://www.aegisub.org/downloads/
https://www.tncnonline.com.vn/Pages/tools.aspx
http://www.ext2fsd.com/?page_id=16
http://www.print-conductor.com/download/

gpedit.msc
msconfig
%appdata%
Phải tinh chỉnh bang tay 1 chút là bỏ hiệu ứng trong Advance System Settings. Bỏ hết tất cả chỉ giữ lại Enable Peek, Show thumnails instead of icons và Smooth edges of screen fonts. Vào msconfig chọn Boot > chọn No GUI boot, chọn Advance options, chọn Number of proccessors và chọn max số CPU.
https://www.quora.com/How-do-I-turn-off-windows-auto-update-in-Windows-8-1
http://www.tomshardware.com/faq/id-1866547/completely-disable-action-center.html
http://www.softwareok.com/?seite=faq-Windows-8&faq=133
http://www.eightforums.com/tutorials/4692-system-restore-how-do-windows-8-a.html
http://winaero.com/blog/how-to-disable-or-enable-fast-startup-in-windows-8-1/
http://www.thewindowsclub.com/disable-defragmentation-ssd
http://www.fixedbyvonnie.com/2015/02/how-to-uninstall-internet-explorer-windows-8-1/
http://ictnews.vn/the-gioi-so/thu-thuat/dat-ten-folder-la-1-khoang-trang-11370.ict
Alt + 0, 1, 6, 0
http://www.thewindowsclub.com/disable-data-execution-prevention
http://vnhow.vn/howto/cach-them-recycle-bin-vao-my-computer-tren-windows-vista-hoac-windows-7
http://www.askvg.com/fix-input-indicator-icon-comes-back-in-taskbar-notification-area-after-restarting-windows/
http://winaero.com/blog/how-to-create-shortcut-for-shutdown-windows-dialog/
dim objShell
set objShell = CreateObject("shell.application")
objshell.ShutdownWindows
set objShell = nothing
Change name all file .exe in folder "Internet Explorer" and "Windows Media Player"
http://helpdeskgeek.com/windows-7/windows-7-how-to-delete-files-protected-by-trustedinstaller/
https://stackoverflow.com/questions/34218333/pyinstaller-onefile-does-not-start-on-some-windows-7-machines
https://support.microsoft.com/en-us/help/2999226/update-for-universal-c-runtime-in-windows
http://sharecodeweb.net/huong-dan-khac-phuc-khong-download-duoc-tren-app-box-com/
http://chongvirusmaytinh.blogspot.com/2011/05/tat-autoplay-cua-windows-bang-trinh.html
itt.edu.vn/threads/28348-huong-dan-cai-full-driver-cho-windows-8.html
http://www.sony.com.vn/support/product/sve14136cv/download
http://hanoilab.com/v2/huong-dan-recovery-cho-sony-vaio/
http://laptopvaiochinhhang.blogspot.com/2013/01/huong-dan-recovery-sony-vaio-win-8.html
https://support.office.com/en-us/article/Uninstall-Office-2013-Office-2016-or-Office-365-from-a-PC-9dd49b83-264a-477a-8fcc-2fdf5dbf61d8
http://support.microsoft.com/kb/971179
http://gregmaxey.mvps.org/word_tip_pages/customize_ribbon_main.html
https://www.youtube.com/watch?v=kIo5IUxT4Zc&index=19&list=PLs2kLSy9E7-N1iMS-C88BCxceoxJ2-qBI
http://www.youtube.com/watch?v=ivXWe-eX8jI
http://www.youtube.com/watch?v=RG1P4jLX_7s
http://domuc.net/Thu-thuat/Sua-may-in-do-muc-may-in/May-in-Brother-bao-loi-TONER-va-cach-reset-may-in-Brother-nhu-the-nao--254.aspx
http://www.youtube.com/watch?v=s8D-5GdEoJA
http://www.zinetic.co.uk/brother_hl_2240d_drum_unit_reset_instructions-details.aspx
https://sites.google.com/a/maychu.edu.vn/tinhocvht/huong-dan-khac-phuc-su-co/vitinh/may-in/brother/reset-hop-muc-may-in-brother-hl-2250dn
https://khosachdientu.wordpress.com/2012/07/09/huong-dan-chuyen-doi-file-doc-docx-pdf-html-txt-thanh-file-prc-bang-phan-mem-mobipocket-creater-4-2/
https://mongxuans.wordpress.com/2013/06/12/huong-dan-dong-ebook-toan-tap/
http://banlong.us/threads/huong-dan-build-ebook-da-dinh-dang-azw-epub-mobi-prc.10575/
http://quantrimang.com/trai-nghiem-che-do-duyet-web-mobile-tren-may-tinh-94260
http://browserfame.com/723/spoof-user-agent-string-opera-12-wahoo
https://nudegirls4u.com/
http://www.redflava.com
https://hoabanland.wordpress.com/
https://lonelythorn.wordpress.com/
http://jht.pixnet.net/blog
https://vi-vn.facebook.com/ThaiTriHang/
http://tieba.baidu.com/f?kw=凯尔特女人
http://tieba.baidu.com/f?kw=高露
http://tieba.baidu.com/f?kw=戚薇
https://www.facebook.com/StephyQiWeiVN/
http://www.fakenamegenerator.com/
https://login.teamviewer.com/connect/Flash
http://vnoss.org/docs/?id=2
http://sachxua.net/forum/index.php?topic=16946.0

https://www.facebook.com/notes/laptop-huy-hoang/thông-tin-hướng-dẫn-chọn-máy-phù-hợp-hướng-dẫn-mua-hàng-kiểm-tra-thời-gian-bảo-h/381916295281674
https://getsupport.apple.com/ProductSelector.action
http://support.toshiba.com/warranty
http://h10025.www1.hp.com/ewfrf/wc/weInput?cc=us&lc=en
http://h20000.www2.hp.com/bizsupport/TechSupport/WarrantyLookup.jsp?country&prodSeriesId=454811&prodTypeId=12454
http://support.lenovo.com/en_VN/product-service/warranty-status/default.page
http://www.dell.com/support/my-support/us/en/04?c=us&s=bsd&cs&l=en

http://www.cpuid.com/softwares/cpu-z.html
http://crystalmark.info/download/index-e.html
http://www.hwinfo.com/download.php
http://www.techpowerup.com/downloads/SysInfo/GPU-Z/
http://www.softwareok.com/?seite=Microsoft/IsMyLcdOK

gv
http://www.google.com.vn/search?q=%s
g
http://www.google.com/search?q=%s
b
http://www.bing.com/search?q=%s
w
http://en.wikipedia.org/wiki/Special:Search?search=%s
yt
https://www.youtube.com/results?search_query=%s
soku
http://www.soku.com/search_video/q_%s
mp3
http://mp3.zing.vn/tim-kiem/bai-hat.html?q=%s
generasia
http://www.generasia.com/w/index.php?title=Special﷐3ASearch&search=%s&go=Go
imdb
http://www.imdb.com/find?ref_=nv_sr_fn&q=%s&s=all
mydramalist
http://mydramalist.com/browse/all?sort[]=relevance&q=%s
jav
http://www.javlibrary.com/en/vl_searchbyid.php?keyword=%s

tatgiang1@gmail.com; anhngoc7675@gmail.com; huyen3ntl@gmail.com; nguyendinhtb711@gmail.com; namtuliemk2@gmail.com
namtuliem@congdoanhanoi.org.vn
c1namtuliem-ntl@hanoiedu.vn
thanh67.ttb@gmail.com
vukimdung2301@gmail.com
truongnguyen.a@gmail.com
hongttb.1974@gmail.com
dothichinhc1xd@gmail.com
phuonganltt@gmail.com
buithuydung0812@gmail.com
buithihuonghb92@gmail.com
dangminh2910@gmail.com
nguyenthikimthuc1xd@gmail.com
hpduyenpham@gmail.com

http://redbyhbo.com/movie/?view=confession_2014
http://www.redbyhbo.com/movie/?view=prophecy
http://redbyhbo.com/movie/?view=sanpei_the_fisherboy
https://redbyhbo.com/movie/?view=evergreen_love
http://www.redbyhbo.com/movie/?view=the_triumphant_return_of_general_rouge
http://www.redbyhbo.com/movie/?view=have_a_song_on_your_lips
http://www.redbyhbo.com/movie/?view=solomon_s_perjury_part_1_suspicion
http://www.redbyhbo.com/movie/?view=solomon_s_perjury_part_2_judgement

desktop.ini trong Fonts
[.ShellClassInfo]
UICLSID={BD84B380-8CA2-1069-AB1D-08000948F534}

4. Ở khung bên trái, tìm tới khóa sau:
HKEY_CLASSES_ROOT\.exe
5. Ở khung bên phải, thay đổi giá trị của khóa Default thành exefile
Để thay đổi giá trị của khóa Default, bạn nháy đúp vào khóa đó rồi gõ giá trị vào, ấn OK.
6. Ở khung bên trái, tìm tới khóa sau:
HKEY_CLASSES_ROOT\exefile\shell\open\command
Ở khung bên phải, đổi giá trị của khóa Default thành:
“%1″ %*
Xong, giờ đóng Registry Editor lại, khởi động lại máy và thử chạy file EXE xem sao.

[-HKEY_CURRENT_USER\Software\Microsoft\Windows\Curre ntVersion\Explorer\FileExts\.lnk]

Google
  tranqudat@gmail.com (tranqudat@outlook.com; 0918062466)
  milanistavietnam@gmail.com (tranqudat@outlook.com)
  milanistavietnam1991@gmail.com (tranqudat@outlook.com)
  vozvietnam@gmail.com (tranqudat@outlook.com)
  khongcanlakhongcan@gmail.com (tranqudat@outlook.com)
  dulieucuatoi001@gmail.com  Free 1.57GB (tranqudat@gmail.com) (Tong hop + Have a song on your lips)
  dulieucuatoi002@gmail.com  Free 2.45GB (tranqudat@gmail.com) (DVD Believe + DVD Songs from the Heart + Audio Celtic Woman)
  dulieucuatoi003@gmail.com  Free 4.1GB (tranqudat@gmail.com) (Phan mem)
  dulieucuatoi004@gmail.com  Free 2.15GB (tranqudat@gmail.com) (Win)
  dulieucuatoi005@gmail.com  Free 0.37GB (tranqudat@gmail.com) (Nhac)
  dulieucuatoi006@gmail.com  Free 14.19GB (tranqudat@gmail.com) (Nhac Khong chu de)
  dulieucuatoi007@gmail.com  Free 1.55GB (tranqudat@gmail.com) (Chinh nghia xemphimone + Solomon's Perjury)
  dulieucuatoi999@gmail.com
Yahoo
  tranqudat@yahoo.com (tranqudat@gmail; 0918062466) (Flickr)
Microsoft
  tranqudat@outlook.com (tranqudat@gmail.com; 0918062466)
Vn-zoom
  dathv
Voz
  milanistavietnam
  kurtgodel
Hdvietnam
  mrworld
Vn-sharing
  mrworld
Pan baidu
  milanistavietnam@gmail.com
App.box.com
  milanistavietnam@gmail.com
Mega.co.nz
  milanistavietnam@gmail.com
Mediafire
  milanistavietnam@gmail.com

@echo off
start "" "C:\Program Files\Microsoft Office\OFFICE11\WINWORD.EXE"
start "" "C:\Program Files\unikey40RC2-1101-win32\UniKeyNT.exe"

Sub Macro2()
'
' Macro2 Macro
'
'
Dim objPages As Pages
Dim i, n, j, m, truoc, sau
Set objPage = ActiveDocument.ActiveWindow.Panes(1).Pages
j = 0
n = objPage.Count   'Tong so trang cua tai lieu
    
For i = 1 To n - 1 Step 1
    Selection.EndKey Unit:=wdStory   'Di den trang cuoi
    Selection.HomeKey Unit:=wdStory       'Di len trang dau (1)
    Selection.GoTo What:=wdGoToPage, Which:=wdGoToNext, Count:=i + j   'truot den trang thu i + j (1)
    Selection.EndKey Unit:=wdLine, Extend:=wdExtend     'Chon het dong
    Selection.HomeKey Unit:=wdLine                      've dau dong
    truoc = objPage.Count     'SL trang truoc khi chen
    Selection.InsertNewPage                             'Chen trang moi
    sau = objPage.Count       'SL trang sau khi chen
    If truoc = sau Then
        Selection.InsertNewPage                         'Chen trang moi (EP DONG BO)
        sau = objPage.Count   'Cap nhat SL trang sau khi chen ep dong bo
    End If
    j = j + 1
    Selection.EndKey Unit:=wdStory   'Di den trang cuoi
    Selection.HomeKey Unit:=wdStory  'Di len trang dau (1)
Next
Selection.EndKey Unit:=wdStory            'Di den trang cuoi
Selection.InsertNewPage                   'Chen trang moi cho trang cuoi cung
m = MsgBox("XONG", vbOK, "MA LEM KHOANG")
'(1) Luon xuat phat tu trang dau de truot xuong i + j trang
End Sub 

Stylish
https://userstyles.org/styles/102922/remove-outlook-com-ad-bar-and-reclaim-space
Remove Outlook.com Ad bar and Reclaim Space
/**
 * Remove the right ad bar of Outlook.com and reclaim the space
 * @author   Xiangyu Bu <xybu92@live.com>
 */

@-moz-document domain(live.com) {
    #RightRailContainer {display: none;}
    .WithRightRail {right: 0 !important;}
}
Outlook Mail
@-moz-document regexp("https?://(.*\\.)?(outlook|mail)\\.live\\.com/.*") {
#primaryContainer div[style*="width: 165px;"]{display:none!important;}

#primaryContainer div[style*="right: 165px;"]{right:0!important;}
}
Google

@-moz-document domain(google.com.vn),domain(google.com) {

.rc{
background: #F3F3F3 !important; min-width: 600px !important;
padding: 15px !important; border-radius: 3px !important;
}
.rc, .rg_ic.rg_i, #extrares , #foot  {box-shadow: 0 2.5px 2.5px #c3c3c3 !important}

.f.kv._SWb{ border-bottom: #e6e6e6 1px solid ; margin-bottom: 8px !important }
#main {background: #e6e6e6 !important} #appbar {background: #F3F3F3} 
#foot {background: rgba(255,255,255,0.4) !important}
    
/*  Font chữ Title */    
 .r a {text-decoration:none !important; font-family: Segoe UI }
.st,  ._nd, .action-menu.ab_ctl {font-size : 13.1px; font-family : Verdana}
#rso {margin-top: 8px !important}    


#slim_appbar {margin-top:-8px;margin-left:-12px}
#resultStats{margin-top:9px !important; margin-left: 20px}
.sfibbbc {min-width: 634px; margin-left: 15px}

}
Vozforums.com
/* vOz*/

@-moz-document domain(vozforums.com) {
  
.page {background: #5C7099}  [style*=wrap] > STRONG {margin-right:100px} 
[id^="thread_title"] {font-size:12.324px} A[style="white-space:nowrap"] {font-weight:700} [id^="post_message"], 
DIV[style="font-style:italic"], .alt2[style="border:1px inset"], LI, .bodytext {font-size:13.5px; line-height:25px !important} 
A[rel="nofollow"][href^="member.php?"] {font-size:11px} 

/* Fix vụ tràn hình  */
.voz-post-message img {max-width: 900px }     
    
/* Làm màu khung Reply */
.panel ,  .fieldset,  .vBulletin_editor, .button, [id$=smiliebox] {background: #E4E7F5; border:none}
.button {height: 30px} .button:hover, .imagebutton * {background: #F2F2FF} 

/* Banh rộng khung soạn thảo */
DIV[style="width:640px"], [type="text/javascript"] + TABLE, #vB_Editor_001_textarea {min-width:100% }
}
Firefox

/* Cho nút Reload|Stop ra kế Back/Forward */
#urlbar-reload-button, #urlbar-stop-button { -moz-box-ordinal-group: 0; padding-top: 0px; padding-left: 10px !important}
#urlbar-go-button {display:none !important}

/* Kích cỡ Font chữ trên URLbar + Searchbar */
#urlbar * , #searchbar * {font-size:14px !important}

/* Close Tab Button */
.tab-close-button {
	visibility: hidden !important;
	margin-left: -16px !important;
}

.tabbrowser-tab:not([selected]):hover .tab-close-button {
	visibility: visible !important;	
	margin-left: 0px !important;
}

/* Addon Manager */
@-moz-document url(about:addons) {#category-discover {display:none }}

/* Context Menu
http://forums.mozillazine.org/viewtopic.php?f=38&t=2786537
http://forums.mozillazine.org/viewtopic.php?f=38&t=192879
http://kb.mozillazine.org/Chrome_element_names_and_IDs */
menupopup > menu, menupopup > menuitem,
popup > menu, popup > menuitem
{ font-family: Segoe UI !important;  
font-size: 14pt !important }
#context-openlink,
#context-openlinkincurrent,
#context-openlinkprivate,
#context-bookmarklink,
#context-savelink,
#context-sendlink,
#context-copyemail,
#context-sendimage,
#context-setWallpaper,
#context-setDesktopBackground,
#context-blockimage,
#context-back,
#context-forward,
#context-reload,
#context-stop,
#context-bookmarkpage,
#context-savepage,
#context-sendpage,
#context-viewimageinfo,
#context-viewbgimage,
#context-selectall,
#context-keywordfield,
#context-searchselect,
#frame,
#context-viewpartialsource-selection,
#context-viewpartialsource-mathml,
#context-viewinfo,
#context-metadata,
#context-bidi-text-direction-toggle,
#context-bidi-page-direction-toggle,
#spell-add-dictionaries,
#spell-add-dictionaries-main,
#context-sep-open,
#context-sep-copylink,
#context-sep-copyimage,
#context-sep-stop,
#context-sep-viewbgimage,
#context-sep-undo,
#context-sep-paste,
#context-sep-selectall,
#frame-sep,
#context-sep-properties,
#context-sep-bidi {
  display: none !important;
}

/* Findbar on Top Right */
@namespace url("http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul");
@-moz-document url("chrome://browser/content/browser.xul"),
               url("chrome://global/content/viewSource.xul") {
  :-moz-any(.browserContainer, #viewSource) .findbar-container > *,
  :-moz-any(.browserContainer, #viewSource) .findbar-container > *:not(spacer),
  :-moz-any(.browserContainer, #viewSource) :-moz-any(
    .findbar-find-previous, .findbar-find-next
  ) {
    display: -moz-box
  }

  :-moz-any(.browserContainer, #viewSource) .findbar-container :-moz-any(
    spacer, .findbar-find-fast[anonid="match-case-status"]
  ) {
    visibility: collapse
  }

  :-moz-any(.browserContainer, #viewSource) .findbar-find-fast.findbar-find-status {
    -moz-box-ordinal-group: 0;
    margin: 0 1em !important;
  }

  :-moz-any(.browserContainer, #viewSource) .findbar-closebutton {
    position: relative !important;
    right: 0 !important;
  }

  :-moz-any(.browserContainer, #viewSource) .findbar-textbox {
    font-size: medium !important;
  }
  
  .browserContainer findbar,
  #viewSource #FindToolbar {
    -moz-box-ordinal-group: 0;
    position: fixed !important;
    right: 20px;
    margin-top: -1px !important;
    box-shadow: 1px 2px 3px rgba(0, 0, 0, .25) !important;
    border: 1px solid threedshadow !important;
    border-top: none !important;
    border-radius: 0 0 3px 3px;
    transition-property: margin-top, opacity, visibility !important;
  }

  #viewSource #FindToolbar {
    background-color: -moz-dialog;
    color: -moz-dialogtext;
  }
  :-moz-any(.browserContainer, #viewSource) .findbar-find-status { color: inherit !important }

  #main-window:-moz-any(:not([stylish-application="Firefox"]),
    [stylish-application="Firefox"][stylish-application-version^="1"],
    [stylish-application="Firefox"][stylish-application-version^="20"],
    [stylish-application="Firefox"][stylish-application-version^="21"],
    [stylish-application="Firefox"][stylish-application-version^="22"],
    [stylish-application="Firefox"][stylish-application-version^="23"],
    [stylish-application="Firefox"][stylish-application-version^="24"])
  #FindToolbar:after {
    background-color: -moz-dialog;
    color: -moz-dialogtext;
    border: 1px solid threedshadow;
    border-radius: 5px;
    box-shadow: 2px 3px 6px rgba(0, 0, 0, .5);
    position: fixed;
    width: 20em;
    height: 3em;
    padding: 1em 1em;
    left: -moz-calc(50% - 10em);
    top: -moz-calc(50% - 2.5em);
    font-size: x-large;
    font-weight: bold;
    line-height: 1.5em;
    text-align: center;
    pointer-events: none;
    content: "Findbar on Top Right user style only works with Firefox 25 and newer";
  }

  /* MozillaZine */
  #main-window[stylish-domain$="mozillazine.org"] .browserContainer findbar {
    background: url("http://forums.mozillazine.org/static/common/images/sky2.png")
                100% 0 !important;
  }
  #main-window[stylish-domain$="mozillazine.org"] .browserContainer :-moz-any(
    findbar, .tabbable, .findbar-find-status
  ) {
    color: black !important;
  }

  /* Kaskus */
  #main-window[stylish-domain$="kaskus.co.id"] .browserContainer findbar {
    background: -moz-linear-gradient(50% 0%, rgb(20, 132, 206) 0%,
                                             rgb(18, 116, 181) 100%) !important;
  }
  #main-window[stylish-domain$="kaskus.co.id"] .browserContainer :-moz-any(
    findbar, .tabbable, .findbar-find-status
  ) {
    color: #fff !important;
  }
}

B1: Rút hộp mực ra khỏi máy. Mở nguồn máy in.
B2: bên tay trái của mình (nhìn trực diện máy in) bên trong thân máy chỗ đường rãnh cho hộp mực vào có 1 cái lò so to bằng đầu đũa gắn nẫy (1) nhỏ, cái nẫy này nó tỳ vào bánh răng hộp mực. Các bạn lấy tay ấn vào cái nẫy đấy.
B3: Bên tay phải của mình (nhìn trực diện máy in) có 1 lỗ bên trong có nẫy (2) contac ( nhìn nắp đậy sẽ có 1 cái ngạm nhựa dài ra vừa đủ khớp để chọt vào lỗ nói trên dùng để báo nắp đậy của cửa máy in). Chèn 1 cây que vào đẩy cái nẫy (2) giữ yên để đánh lừa cho máy xác nhận đã đóng nắp.
B4: Vừa làm B3 xong máy in sẽ khởi động cơ, lúc đầu tiếng "rồ" nhỏ, sau đó sẽ phát tiếng " rồ" lớn.-> đếm 1-2-3 với tốc độ đếm của tín hiệu đèn đường ( lúc này tiếng "rồ" lớn vẫn kêu), ta thả tay trài ra khỏi nẫy (1) với thời gian 1 giây lại giữ nẫy (1) lại -> Máy ngưng "rồ" lớn 1-2 giây rồi lại rồ lớn lần nữa -> Máy sẽ không còn tín hiệu toner.
B5: khi redy đã báo sáng redy thì ta rút hết tay ra. cho hộp mực vào đậy nắp. Bật nguồn. Thế là ok.
